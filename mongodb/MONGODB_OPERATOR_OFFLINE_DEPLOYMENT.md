...
chmod +x mongodb/scripts/prepare-mongodb-operator-bundle.sh

# Architecture is auto-detected from 'uname -m'
./mongodb/scripts/prepare-mongodb-operator-bundle.sh

# Or specify explicitly if targeting a different arch than your prep machine
./mongodb/scripts/prepare-mongodb-operator-bundle.sh --arch arm64   # for arm64 clusters
./mongodb/scripts/prepare-mongodb-operator-bundle.sh --arch amd64   # for x86_64 clusters
```
...
scp mongodb/scripts/install-mongodb-operator.sh ubuntu@192.168.64.17:~/
...
./mongodb/scripts/prepare-mongodb-operator-bundle.sh --arch arm64
...
### Issue: One MongoDB pod stuck in `Pending` (anti-affinity)

**Cause:** The operator sets `podAntiAffinity` by default — it refuses to schedule two MongoDB pods on the same node. With 3 replicas and only 2 worker nodes, the third pod can never be placed.

**Diagnose:**
```bash
kubectl describe pod my-mongodb-cluster-rs0-2 -n mongodb
# Look for FailedScheduling event:
# "0/2 nodes available: 2 node(s) didn't match pod anti-affinity rules"
```

**Fix — disable anti-affinity for the affected replset:**
```bash
kubectl patch psmdb my-mongodb-cluster -n mongodb \
  --type=merge \
  -p '{"spec":{"replsets":[{"name":"rs0","affinity":{"antiAffinityTopologyKey":"none"}}]}}'
```

> This allows multiple MongoDB pods to share the same node. Suitable for dev/lab clusters with fewer nodes than replicas. In production, prefer having at least as many nodes as replicas.

---

### Issue: Cluster stuck in `initializing` after image change (operator deadlock)

**Cause:** A pod gets `ImagePullBackOff` (wrong or non-existent image tag) and stays in that state. The operator sees the cluster as `initializing` and stops reconciling — it will not roll out any further changes, including a fix, until the stuck pod is removed.
...
# Check pod status
kubectl get pods -n mongodb
# my-mongodb-cluster-rs0-0   0/1   ImagePullBackOff

# Confirm the image tag does not exist
kubectl describe pod my-mongodb-cluster-rs0-0 -n mongodb
# Events: Failed to pull image "...": not found
```

**Fix:**
1. Patch the CR to a valid image tag that is loaded on the nodes:
   ```bash
   kubectl patch psmdb my-mongodb-cluster -n mongodb \
     --type=merge \
     -p '{"spec":{"image":"percona/percona-server-mongodb:7.0.30-16"}}'
   ```
2. Force the stuck pod to restart so the operator can reconcile:
   ```bash
   kubectl delete pod my-mongodb-cluster-rs0-0 -n mongodb
   ```
3. The operator will now reconcile and recreate the pod with the correct image.

> **Root cause prevention:** Always verify an image tag exists in containerd on every node before patching `spec.image`. Use `sudo k3s ctr --namespace k8s.io images list | grep <tag>`.
...
### Scheduled Backups with Percona Backup for MongoDB

The operator has built-in backup scheduling via the `backup.tasks` field in the cluster spec — no external cron jobs or scripts needed. Schedules are defined once and the operator runs them automatically.

> ⚠️ **Offline cluster requirement:** The backup image `perconalab/percona-backup-mongodb:2.13.0` must be loaded on **every node** before enabling backups. Verify with:
> ```bash
> sudo k3s ctr --namespace k8s.io images list | grep percona-backup-mongodb
...
**Step 1: Enable backup storage and schedule in the cluster spec**

```bash
kubectl -n mongodb patch psmdb my-mongodb-cluster --type=merge -p '{
  "spec": {
    "backup": {
      "enabled": true,
      "image": "perconalab/percona-backup-mongodb:2.13.0",
      "storages": {
        "local-backup": {
          "type": "filesystem",
          "filesystem": {
            "path": "/var/lib/mongodb-backup"
          }
        }
      },
      "tasks": [
...
**Step 2: Verify backup is configured**

```bash
kubectl -n mongodb get psmdb my-mongodb-cluster \
  -o jsonpath='{.spec.backup.tasks}' | python3 -m json.tool
```

**Step 3: Trigger a manual backup on demand**
...
# NAME                CLUSTER              STORAGE        STATUS   AGE
# manual-backup-now   my-mongodb-cluster   local-backup   ready    3m
# daily-2am-...       my-mongodb-cluster   local-backup   ready    8h
```
...
1. Open the **`>_` kubectl shell** in Rancher (top right of cluster view)

2. **Enable backup storage and schedule:**
...
4. **Monitor backup status in Rancher UI:**
   - Go to **☰ → Cluster → More Resources → psmdb.percona.com → PerconaServerMongoDBBackups**
   - Each backup shows `STATUS: ready` when complete ✅
...
kubectl -n mongodb exec -it my-mongodb-cluster-rs0-0 -- \
  mongosh -u userAdmin -p 'UserAdminPassword123!' \
...
## Upgrading the Operator (Offline)

Use the upgrade workflow when a new version of the Percona MongoDB Operator or MongoDB is available and you need to apply it to an air-gapped cluster.

### Overview

```
ONLINE MACHINE                          OFFLINE K3s CLUSTER
──────────────────────────────          ──────────────────────────────────
prepare-upgrade-bundle.sh               upgrade-mongodb-operator.sh
  --from-operator 1.21.0     →tar.gz→   --bundle upgrade-bundle-1.22.0.tar.gz
  --to-operator   1.22.0                 --dry-run  (optional preview)
  --arch          arm64
```

### Step 1: Prepare the Upgrade Bundle (online machine)

```bash
# Operator-only upgrade
./mongodb/scripts/prepare-upgrade-bundle.sh \
  --from-operator 1.21.0 \
  --to-operator   1.22.0 \
  --arch          arm64

# Operator + MongoDB image upgrade (any version pair)
./mongodb/scripts/prepare-upgrade-bundle.sh \
  --from-operator 1.21.0 \
  --to-operator   1.22.0 \
  --from-mongodb  7.0.25-14 \
  --to-mongodb    7.0.30-16 \
  --arch          arm64

# Multiple MongoDB version pairs in one bundle
./mongodb/scripts/prepare-upgrade-bundle.sh \
  --from-operator 1.21.0 \
  --to-operator   1.22.0 \
  --from-mongodb  6.0.27-21 --to-mongodb 7.0.30-16 \
  --from-mongodb  7.0.25-14 --to-mongodb 7.0.30-16 \
  --arch          arm64
```

Output: `offline-bundle/mongodb-upgrade/upgrade-bundle-1.22.0.tar.gz`

### Step 2: Transfer to Offline Cluster

```bash
scp offline-bundle/mongodb-upgrade/upgrade-bundle-1.22.0.tar.gz \
    ubuntu@<cluster-node-ip>:/opt/
```

### Step 3: Preview Changes (optional)

```bash
sudo ./mongodb/scripts/upgrade-mongodb-operator.sh \
...
> **Non-default namespace?** Always pass `--namespace <name>`. The default is `mongodb`; if your operator is installed in a different namespace (e.g. `syskey`), omitting this will cause the script to target the wrong namespace.

The script automatically:
1. Validates the installed version matches the bundle's `FROM_VERSION`
2. Captures a rollback snapshot at `/var/log/k3s-install/rollback/<timestamp>/`
3. Loads new images into K3s containerd
4. Applies updated CRDs
5. Runs `helm upgrade` for the operator
6. Patches MongoDB CR images
7. Waits for cluster to reach `ready` state
...
If an upgrade needs to be reversed, use the rollback snapshot captured automatically by the upgrade script.

### Find the Snapshot

```bash
ls /var/log/k3s-install/rollback/
# Example output: 2026-06-02-10-30/
```

The snapshot path is also printed prominently in the upgrade script output.

### Run the Rollback

```bash
sudo ./mongodb/scripts/rollback-mongodb-operator.sh \
  --rollback-dir /var/log/k3s-install/rollback/2026-06-02-10-30 \
  --namespace    mongodb
```

> **Non-default namespace?** Pass the same `--namespace` you used during upgrade.

The rollback script restores in this mandatory order:
1. **Operator** — `helm rollback` to saved revision (uses Kubernetes secrets — works offline, no internet needed). Falls back to `helm upgrade --install` from `old-chart.tgz` only if Helm history has been purged.
2. **CRDs** — `kubectl apply --server-side --force-conflicts` (with `resourceVersion` stripped to avoid stale-snapshot conflicts)
3. **MongoDB CR** — `kubectl apply --server-side --force-conflicts` restores pre-upgrade image specs; old operator reconciles and triggers rolling restart

> **`old-chart.tgz` missing?** In offline/airgap environments the upgrade script cannot `helm pull` the old chart from the Percona repo, so this file is often absent from the snapshot. The rollback script treats it as a warning (not an error) and proceeds via `helm rollback` instead — which reads from cluster secrets and requires no internet access.

### Rollback Quick Reference (Upgrade Scenarios)

| Scenario | Command |
|----------|---------|
| Undo last upgrade | `sudo ./mongodb/scripts/rollback-mongodb-operator.sh --rollback-dir /var/log/k3s-install/rollback/<timestamp> --namespace <ns>` |
| Preview upgrade before applying | Add `--dry-run` to upgrade command |
| Retry failed upgrade from scratch | Add `--reset-steps` to upgrade command |
| Force upgrade ignoring version check | Add `--force` to upgrade command |
| List rollback snapshots | `ls /var/log/k3s-install/rollback/` |
| Operator deadlock (stuck initializing) | Patch CR to valid image, then `kubectl delete pod <stuck-pod> -n <ns>` |
| Pod stuck Pending (anti-affinity) | `kubectl patch psmdb <name> -n <ns> --type=merge -p '{"spec":{"replsets":[{"name":"rs0","affinity":{"antiAffinityTopologyKey":"none"}}]}}'` |
